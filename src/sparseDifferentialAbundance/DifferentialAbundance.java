...
public class DifferentialAbundance {
	...
	static public<T> T[][][] compute(CompEnv<T> gen, T[][][] inputCounters, T[][][] inputAliceCase, 
		...
		T[][][] outControl = gen.newTArray(5, inputCounters[0].length, 0);
		...
		T[][][] res = gen.newTArray(2, inputCounters[0].length,0);
	...
	public static class Generator<T> extends GenRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) throws Exception {
			...
			for(int i = 0; i < numCounters; i++){
				inputCounters[0][i] = gen.inputOfAlice(Utils.fromFixPoint(i+1.0, width, offset));
			}
			for(int i = 1; i < 4; i++){
				for(int j = 0; j < numCounters; j++){
					inputCounters[i][j] = gen.inputOfAlice(Utils.fromFixPoint(0.0, width, offset));
				}
			}
			...
			for(int i = 0; i < GenCaseNum; i++){
				...
				inputAliceCase[0][i] = gen.inputOfAlice(Utils.fromFixPoint(caseInput[0][i+2], width, offset));
			}
			for(int i = 0; i < GenCaseNum; i++){
				inputAliceCase[1][i] = gen.inputOfAlice(Utils.fromFixPoint(1.0, width, offset));
			}
			for(int i = 0; i < GenCaseNum; i++){
				inputAliceCase[2][i] = gen.inputOfAlice(Utils.fromFixPoint(caseInput[1][i+2], width, offset));
			}
			for(int i = 0; i < GenCaseNum; i++){
				inputAliceCase[3][i] = gen.inputOfAlice(Utils.fromFixPoint(caseInput[1][i+2], width, offset));
			}
			...
			for(int i = 0; i < GenControlNum; i++){
				inputAliceControl[0][i] = gen.inputOfAlice(Utils.fromFixPoint(controlInput[0][i+2], width, offset));
			}
			for(int i = 0; i < GenControlNum; i++){
				inputAliceControl[1][i] = gen.inputOfAlice(Utils.fromFixPoint(1.0, width, offset));
			}
			for(int i = 0; i < GenControlNum; i++){
				inputAliceControl[2][i] = gen.inputOfAlice(Utils.fromFixPoint(controlInput[1][i+2], width, offset));
			}
			for(int i = 0; i < GenControlNum; i++){
				inputAliceControl[3][i] = gen.inputOfAlice(Utils.fromFixPoint(controlInput[1][i+2], width, offset));
			}
		...
		public void prepareOutput(CompEnv<T> gen) {
			...
			for(int i = in.length-1; i < in.length; i++){
				for(int j = 0; j < in[0].length; j++){
					System.out.print(Utils.toFloat(gen.outputToAlice(in[i][j]), 24, 8) + " ");
				}
			}
			*/
	...
	public static class Evaluator<T> extends EvaRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) throws Exception {
			...
			for(int i = 0; i < EvaCaseNum; i++){
				inputBobCase[0][i] = gen.inputOfBob(Utils.fromFixPoint(caseInput[0][i+2], width, offset));
			}

			for(int i = 0; i < EvaCaseNum; i++){
				inputBobCase[1][i] = gen.inputOfBob(Utils.fromFixPoint(1.0, width, offset));
			}
			for(int i = 0; i < EvaCaseNum; i++){
				inputBobCase[2][i] = gen.inputOfBob(Utils.fromFixPoint(caseInput[1][i+2], width, offset));
			}

			for(int i = 0; i < EvaCaseNum; i++){
				inputBobCase[3][i] = gen.inputOfBob(Utils.fromFixPoint(caseInput[1][i+2],width, offset));
			}
			gen.flush();
			...
			for(int i = 0; i < EvaControlNum; i++){
				inputBobControl[0][i] = gen.inputOfBob(Utils.fromFixPoint(controlInput[0][i+2], width, offset));
			}
			for(int i = 0; i < EvaControlNum; i++){
				inputBobControl[1][i] = gen.inputOfBob(Utils.fromFixPoint(1.0, width, offset));
			}
			for(int i = 0; i < EvaControlNum; i++){
				inputBobControl[2][i] = gen.inputOfBob(Utils.fromFixPoint(controlInput[1][i+2], width, offset));
			}
			for(int i = 0; i < EvaControlNum; i++){
				inputBobControl[3][i] = gen.inputOfBob(Utils.fromFixPoint(controlInput[1][i+2], width, offset));
			}
