...
public class SparseDAFixedPoint {
	static public int PLength = 20;
	static public int VLength = 30;
	static public<T> T[][] compute(CompEnv<T> gen, T[][][] inputCounters, T[][][] inputAliceCase, T[][][] inputAliceControl, 
		...
		FixedPointLib<T> flib = new FixedPointLib<T>(gen, VLength, PLength);
		...
		for(int i = (inCase[0].length-2); i >= 0; i--){
			...
			T[] result = flib.multiply(first, second);
			inCase[2][i] = flib.add(result, inCase[1][i]);
			inCase[3][i] = flib.add(flib.multiply(inCase[1][i+1], inCase[3][i+1]), flib.multiply(inCase[3][i], inCase[3][i]));
		...
		for(int i = (inControl[0].length-2); i >= 0; i--){
			...
			T[] result = flib.multiply(first, second);
			inControl[2][i] = flib.add(result, inControl[1][i]);
			inControl[3][i] = flib.add(flib.multiply(inControl[1][i+1], inControl[3][i+1]), flib.multiply(inControl[3][i], inControl[3][i]));
	...
	public static class Generator<T> extends GenRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) throws Exception {
			...
			numAliceCase = gen.inputOfAlice(Utils.fromFixPoint(caseInput[0][0], VLength, PLength));
			numAliceControl = gen.inputOfAlice(Utils.fromFixPoint(controlInput[0][0], VLength, PLength));
			gen.flush();
			...
			for(int i = 0; i < numCounters;i++){
				inputCounters[0][i] = gen.inputOfAlice(Utils.fromFixPoint(i+1, VLength, PLength));
			}
			for(int i = 0; i < numCounters;i++){
				inputCounters[1][i] = gen.inputOfAlice(Utils.fromFixPoint(0.0, VLength, PLength));
			}
			for(int i = 0; i < numCounters;i++){
				inputCounters[2][i] = gen.inputOfAlice(Utils.fromFixPoint(0.0, VLength, PLength));
			}
			for(int i = 0; i < numCounters;i++){
				inputCounters[3][i] = gen.inputOfAlice(Utils.fromFixPoint(0.0, VLength, PLength));
			}
			gen.flush();
			
			inputAliceCase = gen.newTArray(counterRows, GenCaseCounts, l.length);
			for(int i = 0; i < GenCaseCounts; i++){
				inputAliceCase[0][i] = gen.inputOfAlice(Utils.fromFixPoint(caseInput[0][i+2], VLength, PLength));
			}
			for(int i = 0; i < GenCaseCounts; i++){
				inputAliceCase[1][i] = gen.inputOfAlice(Utils.fromFixPoint(1.0, VLength, PLength));
			}
			for(int i = 0; i < GenCaseCounts; i++){
				inputAliceCase[2][i] = gen.inputOfAlice(Utils.fromFixPoint(caseInput[1][i+2], VLength, PLength));
			}
			for(int i = 0; i < GenCaseCounts; i++){
				inputAliceCase[3][i] = gen.inputOfAlice(Utils.fromFixPoint(caseInput[1][i+2], VLength, PLength));
			}
			gen.flush();
			
			inputAliceControl = gen.newTArray(counterRows, GenControlCounts, l.length);
			for(int i = 0; i < GenControlCounts; i++){
				inputAliceControl[0][i] = gen.inputOfAlice(Utils.fromFixPoint(controlInput[0][i+2], VLength, PLength));
			}
			for(int i = 0; i < GenControlCounts; i++){
				inputAliceControl[1][i] = gen.inputOfAlice(Utils.fromFixPoint(1.0, VLength, PLength));
			}
			for(int i = 0; i < GenControlCounts; i++){
				inputAliceControl[2][i] = gen.inputOfAlice(Utils.fromFixPoint(controlInput[1][i+2], VLength, PLength));
			}
			for(int i = 0; i < GenControlCounts; i++){
				inputAliceControl[3][i] = gen.inputOfAlice(Utils.fromFixPoint(controlInput[1][i+2], VLength, PLength));
			}
			gen.flush();
		...
		public void prepareOutput(CompEnv<T> gen) {
			//for(int i = 0; i < ret.length; i++){
				for(int j = 0; j < ret.length; j++){
					...
					System.out.print(Utils.toFixPoint(gen.outputToAlice(ret[j]), PLength) + " ");
				}
				System.out.println();
			//}	
		}
	...
	public static class Evaluator<T> extends EvaRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) throws Exception {
			...
			numBobCase = gen.inputOfBob(Utils.fromFixPoint(caseInput[0][0], VLength, PLength));
			numBobControl = gen.inputOfBob(Utils.fromFixPoint(controlInput[0][0], VLength, PLength));
			gen.flush();
			...
			for(int i = 0; i < EvaCaseCounts; i++){
				inputBobCase[0][i] = gen.inputOfBob(Utils.fromFixPoint(caseInput[0][i+2], VLength, PLength));
			}
			for(int i = 0; i < EvaCaseCounts; i++){
				inputBobCase[1][i] = gen.inputOfBob(Utils.fromFixPoint(1.0, VLength, PLength));
			}
			for(int i = 0; i < EvaCaseCounts; i++){
				inputBobCase[2][i] = gen.inputOfBob(Utils.fromFixPoint(caseInput[1][i+2], VLength, PLength));
			}
			for(int i = 0; i < EvaCaseCounts; i++){
				inputBobCase[3][i] = gen.inputOfBob(Utils.fromFixPoint(1.0, VLength, PLength));
			}
			gen.flush();
			
			inputBobControl = gen.newTArray(counterRows, EvaControlCounts, l.length);
			for(int i = 0; i < EvaControlCounts; i++){
				inputBobControl[0][i] = gen.inputOfBob(Utils.fromFixPoint(controlInput[0][i+2], VLength, PLength));
			}
			for(int i = 0; i < EvaControlCounts; i++){
				inputBobControl[1][i] = gen.inputOfBob(Utils.fromFixPoint(1.0, VLength, PLength));
			}
			for(int i = 0; i < EvaControlCounts; i++){
				inputBobControl[2][i] = gen.inputOfBob(Utils.fromFixPoint(controlInput[1][i+2], VLength, PLength));
			}
			for(int i = 0; i < EvaControlCounts; i++){
				inputBobControl[3][i] = gen.inputOfBob(Utils.fromFixPoint(1.0, VLength, PLength));
			}
			gen.flush();
