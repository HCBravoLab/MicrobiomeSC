...
public class SparseOddsRatio {
	static public<T> T[][] compute(CompEnv<T> gen, T[][][] inputCounters, T[][][] inputAliceCase, T[][][] inputAliceControl, 
		...
		for(int i = (in1Case.length-2); i >= 0; i--){
			in3Case[i] = ilib.add(in2Case[i], ilib.multiply(in2Case[i+1], in3Case[i+1]));
		}
		...
		System.arraycopy(in1Control, 0, inControl[0], 0, in1Control.length);
		System.arraycopy(in2Control, 0, inControl[1], 0, in2Control.length);
		System.arraycopy(in3Control, 0, inControl[2], 0, in3Control.length);
		
		//make accumulation pass
		// s[i][2] = s[i][1] + s[i+1][1]*s[i+1][2]
		
		for(int i = (in1Control.length-2); i >= 0; i--){
			in3Control[i] = ilib.add(in2Control[i], ilib.multiply(in2Control[i+1], in3Control[i+1]));
		}
	...
	public static class Generator<T> extends GenRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) throws Exception {
			...
			System.out.print("gen control input : "); 
			for(int i = 0; i < controlInput.length; i++){
			System.out.print(controlInput[i] + " ");
			}
			System.out.println();

			System.out.print("gen case input : "); 
